...
public class Cast {
	 ...
	 * 형변환 : 값의 자료형을 바꾸는 것
	 * 
	 * * 컴퓨터에서 값 처리 규칙(형 변환이 필요한 상황)
	 * 1) 대입 연산자 기준으로 왼쪽과 오른쪽이 같은 자료형이어야함.
	 *  => 같은 자료형에 해당하는 값만 대입가능.
	 ...
	 * 
	 * * 형변환 종류
	 * 1) 자동형변환 => 자동으로 형변환이 되어서 우리가 따로 형변환 x
	...
	public static void autoCasting() {
		...
		double result = /*(double)*/n2 + d2;
		System.out.println(result);
		
		// 3) float(4byte) => double(8byte)
		float f1 =1.5f; //float 자료형의 값은 값 뒤에 f를 붙여야함. 안붙여주면 double형으로 인식해서 오류
		double d3 = /*(double)*/f1; //크기가 큰 방향으로 자동형변환됨.
		System.out.println(d3);
		
		// 4) long(8byte) => float(4byte)
		//	  *float이 실수형이기 때문에 long형보다 표현 가능한 범위가 더 커서 자동형변환됨.
		long l4 = 10000000000L; //long 자료형의 값은 값 뒤에 l을 붙여야함
		float f4 = /*(float)*/l4;
		...
		char ch = 91;
		System.out.println(ch); //아스키코드 ASCII Code
		
		// 각 문자들이 고유한 숫자가 지정되어 있어 서로 형변환이 가능 (아스키 코드)
	...
	public static void forceCasting() {
		...
		//int rNum = num + dNum; //10.0 + 3.14 => 13.14(double)
								 //연산 결과가 double 형이라서 int형에 대입 불가
		//해결 방법
		int rNum = (int)(num + dNum); //10 + 3 (데이터 손실)
