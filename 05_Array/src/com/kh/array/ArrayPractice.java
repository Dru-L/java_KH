...
public class ArrayPractice {
	/*
	 * 배열 : 같은 자료형의 여러개의 데이터를 저장하는 공간
	 *
	 * (1차원)배열
	 * [선언 방법]
	 * 자료형 [] 배열 이름 = new 자료형[데이터길이];
	 * 
	 ...
	 * 			 배열A[i] = 배열B[i];	//배열B의 i번째 데이터를 배열A의 i번째 데이터로 복사(대입)
	 * 		 }
	 ...
	 *  ** foreach라는 키워드를 사용하는 것이 아니라, 다른 언어에서 사용하는 foreach와 구조가 유사해서 보통 foreach라고 부름
	 *  [사용 방법]
	 *  for(자료형 변수명 : 배열 이름) {
	...
	public void practice3() {
		...
		System.out.println("원본배열(nums)의 주소값: "+nums);
		System.out.println("복사 후 배열(copy)의 주소값: "+copy);
		
		//두 배열 데이터를 출력
		//Arrays.toString(배열이름); => 배열의 데이터를 [값1, 값2,..]형태로 반환
		System.out.println("원본 배열(nums)데이터 : "+Arrays.toString(nums));
		System.out.println("복사 후 배열(copy)데이터 : "+Arrays.toString(copy));
		
		//원본 배열에서(nums) 두번째 자리에 값을 + 10
		nums[1] += 10;
		
		System.out.println("...원본 배열에서(nums) 두번째 자리에 값을 + 10");
		System.out.println("원본 배열(nums)데이터 : "+Arrays.toString(nums));
		System.out.println("복사 후 배열(copy)데이터 : "+Arrays.toString(copy));
		...
		// ※ 얕은 복사는 원본 배열의 값을 변경하거나 복사본 배열의 값을 변경하면 서로에게 영향을 준다.
	}
	...
	public void practice4() {
		...
		System.out.println("원본배열(prices)의 주소값: "+prices);
		System.out.println("복사 후 배열(discountPrices)의 주소값: "+discountPrices);
		
		//[1]반복문 사용
		for(int i=0; i<prices.length; i++) {
			//prices 배열의 i 번째 데이터를
		...
		System.out.println("원본 배열(prices)데이터 : "+Arrays.toString(prices));
		System.out.println("복사 후 배열(discountPrices)데이터 : "+Arrays.toString(discountPrices));
		
		System.out.println("----20% 할인 후----");
		...
		// ※깊은 복사는 원본 배열의 값을 변경하거나 복사한 배열의 값을 변경하더라도 서로에게 영향을 주지 않는다.
	}
}
